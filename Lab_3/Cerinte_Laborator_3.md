...
1. Care sunt cele 4 metode HTTP principale și când se folosește fiecare?
   ...
   * **HEAD** - se comporta exact ca metoda GET, dar serverul returneaza doar antetul resursei, ceea ce permite clientului sa inspecteze
...
2. Ce semnifică codurile de status: 200, 301, 400, 401, 403, 404, 500?
   * **200 - "ok"**, cererea a fost executata cu succes. Informatia a revenit cu un raspuns pozitiv, indiferent de modul in care s-a facut cererea. (**2xx - raspuns reusit**, clasa de raspuns/status ce indica utilizatorului ca cererea a fost primita, inteleasa si acceptata cu succes)
   * **301 - "modificat/mutat permanent"**, cererii i-a fost atribuita o sursa noua si permananta URI iar cererile urmatoare ar trebui sa foloseasca una din sursele returnate de URI (**3xx - redirectionari**,  clasa de raspuns/status indica faptul ca actiunile urmatoare vor fi facute de browser pentru a putea fi indeplinita cererea. )
   * **400 - "cerere gresita"**, cererea nu a putut fi inteleasa/perceputa de catre server din cauza unei sintaxe grsite/incomplete. utilizatorul ar trebui sa nu repete cerea fara ca aceasta sa suporte modificari (
   * **4xx - erori ale utilizatorilor**, clasa de mesaje/statusuri este folosita in cazurile in care utilizatorul ar putea gresi formularea cererii)
   * **401 - "neautorizat"**, cererea necesita autentificare/ inregistrarea utilizatorului.
   * **403 - "interzis"**, serverul a inteles cererea, dar refuza sa o indeplineasca. Autorizarea nu ajuta in nici un caz, iar cererea nu ar mai trebui repetata.
   * **404 - "negasit"**, serverul nu a gasit nimic care sa corespunda cererii URI. Nu se dau indicatii despre conditia temporara sau permanenta.
   * **500 - "eroare interna de server"**, serverul a intampinat o conditie neasteptata si o previne spre a putea indeplini cererea. (**5xx - erori de server**, raspunsurile/statusurile ce incep cu 5 indica cazurile in care serverul e constient de greselile produse sau e incapabil sa execute cerera)
3. Care este diferența între HTTP și HTTPS?
   * HTTP - Hyper text transfer protocol
   * HTTP**S** - hyper text transfer protocol **secure -** este o extensie a protocolului HTTP, foloseste criptarea pentru o comunicare securizata cu servarul intr-o retea de calculatoare, si este folosit la scara larga pe Internet. 
...
- Faceți screenshot-uri cu cererile analizate
  - ![1774116620196](image/Cerinte_Laborator_3/1774116620196.png)
...
![1774117586553](image/Cerinte_Laborator_3/1774117586553.png)
